  ...
  <!-- Start Main Top -->
    ...
    <div class="body-content">
      <div class="body-title">
        <h1 ><?php echo $title2[$language_1]; ?>:</h1>
      </div>
          ...
          function edit_function(index)
            {
              ...
              setCookie("name_searched", prod_name, "1");
          ...
          function delete_function(index)
            {
              var result = confirm("Want to delete?");
               if (result) {
                  var temp_prod_name = document.getElementById("name" + index).innerHTML;
                  var temp_prod_cond = document.getElementById("cond" + index).innerHTML;
                  setCookie("prod_name", temp_prod_name, "1");
                  setCookie("prod_cond", temp_prod_cond, "1");
                  var request = new XMLHttpRequest();
                  request.open("GET", "ajax_delete_prod.php?name=" + temp_prod_name, true);
                  request.send();
                  request.onreadystatechange = function() 
              {
                if (this.readyState == 4 && this.status == 200) 
                {
                  ...
                  if(aux == "OK")
                  {
                    alert("The product " + temp_prod_name + " was deleted!");
                    window.location.replace("products.php");
                  }
                  if(aux == "NOK")
                  {
                    alert("The product " + temp_prod_name + " can't be deleted!");
                  }
                }
              };
                }
           ...
           <div class="row">
             <div class="col-8 col-sm-4 col-md-5 ">
              <?php
                ...
                $result = $database->getData($_COOKIE["user"]);
                ...
                while($row = mysqli_fetch_assoc($result))
                {
                  ...
                  ?>  
                   <div class="card">
                    ...
                    <button type="submit" class = "btn btn-dark" id = "edit-card" onclick="edit_function(<?php echo $index ?>)"> <?php echo $card[$language_1][3]; ?> </button>
